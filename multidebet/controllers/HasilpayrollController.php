...
class HasilpayrollController extends \yii\web\Controller {
    ...
    public function actionGethasil()
    {
        ...
        if($model->load($request->post())){
            $data = $searchModel->searchByDate(date("Ymd",strtotime($model->date_exec)),$co_code);
...
//    public function actionIndexhasil2()
//    {
//        $co_code = Yii::$app->user->identity->branch_cd;
//        $searchModel = new PayrollUploadMaster_search();
...
//                'mode' => Pdf::MODE_CORE, // leaner size using standard fonts
            ...
            'options' => ['title' => 'Hasil Payroll Multidebet'],
            'methods' => [
...
//        ]);
    ...
    public function actionDownloaddetail($id)
    {
        ...
        print "No. Rek Kredit    :" . $modelParam['acctno'] . "\n";
        ...
        {
            $norekBiaya = "Dibebankan Ke Masing-Masing No.Rekening Debet";
        }
        else
        {
...
//        print "No. Rek Biaya     :".$norekBiaya."\n\n";
        ...
        $modelDetail = PayrollUploadDetail::find()->where(['nama_file_upload' => $id])
                ->orderBy('id') // apply sort
                ->all();
        $header = " ID" . " " . "|Kurs" . " " . "|No. Rek. Debet" . " " . "|Nama Rekening" . " " . "|Nominal" . " " . "|Narasi" . " " . "\n";
...
//        
        ...
        $html .= "<table border=0 width=100%>
                    <tr>
                    <td width=5%>" . $data['id'] . "</td>                               //no.baris
                    <td width=15%>" . $data['ft_id'] . "</td>                           //no.FT
                    <td width=15%>" . $data['acctno_cr'] . "</td>                       //no.rek.Debet
                    <td width=15%>" . $data['acct_title'] . "</td>                      //nama rekening
                    <td width=15%>" . $data['date_process'] . "</td>                    //tgl.proses
                    <td width=15%>" . $data['acctno_db'] . "</td>                       //no.rek.Kredit
                    <td width=15%>" . number_format($data['payrollamt'], 2) . "</td>    //nominal debet
                    <td width=15%>" . $data["ft_stat"] . "</td>                         //status FT
                    <td width=15%>" . $data["ft_msg"] . "</td>                          //FT message
                    </tr>
                </table>
                ";
