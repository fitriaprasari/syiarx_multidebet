 ...
 */
namespace app\modules\multidebet\controllers;
...
use app\modules\multidebet\models\PayrollUploadMaster_search;
use app\modules\multidebet\models\PayrollUploadDetailTemp;
...
//        if ($model_master->load(Yii::$app->request->post()))
        {
            ...
            if ($tm_exec_24format > date('20:00:00'))
            {
                $message = "Jam eksekusi tidak boleh lebih dari pukul 20.00";
            ...
            else
            {
                ...
                if ($hasilValidasi == true)
                {
                    ...
                    else
                    {
                        ...
                        $beforeParam = strtok($nama_file,'.');
                        $param = str_replace($beforeParam.'.',"",$nama_file);
...
//                        var_dump($model_master);die;
                        ...
                        if (!$model_master->hasErrors() && $model_master->validate())
                        {
                            ...
                            $folder = Yii::$app->params['multidebet']['upload'];
...
//                        $status = 'error';
                        ...
                        for($i=0;$i < $row; $i++){
                            ...
                            $model_detail_temp->acctno_cr = $model_param['acctno'];
                            $model_detail_temp->acctno_db = $bacaFile[$i]['acctno'];
...
//    }
    ...
    public function actionDaftarupload(){
        ...
        $dataProvider = $searchModel->searchDataUploadNAU(Yii::$app->request->queryParams,$co_code);
//        
...
//                ->where(['co_code'=>$co_code])->all();
        ...
        ->andWhere(['payroll_parameter.tipe_transaksi' => 'MULTIDEBET'])
        ->all();
...
/**
     ...
     */
        ...
        public function actionUpdatedetaildata($id, $nama_file_upload)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate())
        {
            ...
            return $this->redirect(['viewdetaildata', 'id' => $model->id, 'nama_file_upload' => $model->nama_file_upload, 'status'=>$status]);
        }
        else
        {
...
//    
    ...
    protected function findModelDetailTemp($id, $nama_file_upload)
    {
        if (($model = PayrollUploadDetailTemp::findOne(['nama_file_upload' => $nama_file_upload,'id' => $id])) !== null)
        {
            return $model;
        }
        else
    ...
    {
        if (($model = PayrollUploadDetailTemp::findOne(['id' => $id, 'nama_file_upload' => $nama_file_upload])) !== null)
        {
            return $model;
        }
        else
