 ...
 */
namespace app\modules\multidebet\models;
    ...
    public function rules()
    {
        return [
            [['kode_parameter','nama_institusi','charge_flag','tipe_transaksi','acctno',
                'branch_co_code','branch_nm','acctname','rekening_biaya'],'required','on'=>'create'],
            [['kode_parameter','nama_institusi','charge_flag','tipe_transaksi','acctno',
                'branch_co_code','branch_nm','acctname','rekening_biaya'],'required','on'=>'updateparam'],
            [['acctno_charge'],'number'],
            [['acctno'],'string','max'=>16],
//            [['acctno'],'number','when'=> function($model){
//              return $model->rekening_biaya == 'Y';},'on'=>['updateparam','create'],'max'=>16],
//            [['acctno'],'string','when'=> function($model){
//              return $model->rekening_biaya == 'N';},'on'=>['updateparam','create']],
...
//            }, 'on' =>['updateparam','create']],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'acctno' => 'No. Rek. Kredit', //dibalik menjadi rek kredit
            'branch_co_code' => 'Kode Cabang Rek.Kredit', //dibalik menjadi rek kredit
            'branch_nm' => 'Nama Cabang',
            'acctname' => 'Nama Pemilik Rekening',
            'acctno_charge' => 'No. Rek. Biaya',
            'co_code'=>'Kode Cabang Inputter',
            'rekening_biaya'=>'Biaya?',
        ];
    }
...
//     public static function getRekPL($id){
//        //get rekPL dari T24
//        $logmodel = new WsclientLog();
//        $wsdl = Yii::$app->params['wsdlt24'];
//
//        $logmodel->func = "getRekPL";
//        $logmodel->request = $msgreq;
//        $logmodel->inputter = $msgreq['id'];
//        $data = array();
//        $client = new \SoapClient($wsdl);
//        $response = $client->getRekPL(['params' => json_encode($msgreq)]);
//        $msgresp = json_decode($response->return);
//        
