...
//                $log->error_msg = $objresp['params']['OVERRIDE:1:1'];
    ...
    public function pindahBukuPayrollMD($model)
    {
        $ft_stat = '';
        $ft_msg = '';
        $ft_id = '';
...
//        $no_baris = $model->id;
        ...
        $acctno_db = $model->acctno_db;
        $revno = $tgl_upload . "." . $acctno_db . "." . $narasi;

        $retval = array(
            "userid" => Yii::$app->params['payrolluser'],
            "password" => trim(Crypto::Decrypt(Yii::$app->params['payrollpass'])),
            "cocode" => $storcv1->branch_cd,
            "creditacc" => $modelParam->acctno, //menjadi rekening kredit
            "creditcurr" => "IDR",
            "amount" => (string) $model->payrollamt,
            "debitacc" => $model->acctno_db, // menjadi rekening debet
            "debitcurr" => "IDR",
            "ordercust" => "CUSTOMER",
            ...
            "rekening_biaya" => $modelParam->rekening_biaya
...
//        try{
        $client = new \SoapClient(YII::$app->params['wsdlt24'],array('cache_wsdl' => WSDL_CACHE_NONE));
...
//        $message = '';
        ...
        if ($valid_stat == true)
        {
            ...
            {
                //validation acctno
                $acctno_db = $dataupload['acctno_db'];
                $num_acctno_db = count($acctno_db);
                if ($acctno_db == "")
                {
                    $message[0] = "No. Rek. belum diisi";
                    $valid_stat = false;
                }
                else if (is_numeric($acctno_db) == false)
                {
                ...
                else
                {
                    ...
                    $n_acctno = strlen($acctno_db);
                    if (($n_acctno != 10))
                    {
                        $message[0] = "No. Rek. harus 10 digit angka";
                        $valid_stat = false;
                    }
                    else
                    {
                        //check availability of acctno
                        $exist_la = \app\models\LookupAccount::findOne($acctno_db);
                        $message[1]= $exist_la['acct_title'];
//                        $message[1]= $exist_la['acct_title'];
                        if ($exist_la == null)
                        {
                            $exist_a = \app\models\Account::findOne($acctno_db);
